#!/usr/bin/env python3
"""
Evaluate the trained CLIP ViT-L/14 classifier on held-out test clips.

Uses the larger CLIP model for better accuracy.
"""
...
class CLIPLargeEvaluator:
    """Evaluator using MegaDetector + CLIP ViT-L/14 + trained classifier."""
    ...
    def __init__(self, model_path: Path, device: str = "cpu"):
        ...
        # Load CLIP ViT-L/14 for embeddings
        ...
        self.classifier = CLIPClassifierLarge(
        ...
        print(f"Loaded model with {checkpoint.get('best_val_acc', 'N/A'):.1f}% validation accuracy")
    ...
    def extract_embedding(self, crop_bgr: np.ndarray) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            outputs = self.clip_model.get_image_features(pixel_values)
    ...
    def evaluate_video(
        ...
        # Accumulate weighted votes with confidence thresholding
        species_scores = defaultdict(float)
        species_counts = defaultdict(int)
        high_conf_votes = defaultdict(int)
                ...
                if detections is not None and hasattr(detections, 'class_id'):
                    for i, class_id in enumerate(detections.class_id):
                        if class_id == 0:  # animal
                            ...
                            # Weight by confidence squared for stronger emphasis on high-conf
                            species_scores[species] += conf * conf
                            species_counts[species] += 1

                            if conf > 0.5:
                                high_conf_votes[species] += 1
        ...
        if species_scores:
            # Prefer high-confidence votes if available
            if high_conf_votes:
                predicted = max(high_conf_votes.keys(), key=lambda s: high_conf_votes[s])
            else:
        ...
        return {
            ...
            "high_conf_votes": dict(high_conf_votes),
            "top3": sorted_species,
...
def main():
    ...
    model_path = project_dir / "models" / "clip_large_classifier.pt"
    ...
    if not model_path.exists():
        ...
        print("Run train_clip_large.py first to train the model.")
        return 1
    ...
    evaluator = CLIPLargeEvaluator(model_path, device=device)

    print("\n" + "=" * 60)
    print("CLIP ViT-L/14 CLASSIFIER EVALUATION")
    print("=" * 60)
    ...
    for species, clip_names in splits["test"].items():
        ...
        for clip_name in clip_names:
            ...
            if clip_result.get('high_conf_votes'):
                print(f"    High-conf votes: {clip_result['high_conf_votes']}")
    ...
    # Summary
    print("\n" + "=" * 60)
    print("RESULTS SUMMARY (CLIP ViT-L/14 Classifier)")
    print("=" * 60)
    ...
    # Save results
    output_file = script_dir / "evaluation_clip_large_results.json"
